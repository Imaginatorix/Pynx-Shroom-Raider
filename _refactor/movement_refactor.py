...
def firespread(start: tuple, locations: dict[str, set[tuple[int, int]]]) -> None:
    """
        Update the locations dict based on the first tree set on fire.

        Parameters:
            start (tuple): Location of the first tree set on fire.
...
def user_input(curr_map: MapData, orig_map: MapData, sys_input: str = "") -> list[MapData]:
    """
        ...
        Parameters:
            curr_map (MapData): Current map information.
            orig_map (MapData): Original map information, used when "!" is inputted by the user.
            sys_input (str, optional): Moves passed when the main file is ran, Defaults to ""

        Returns:
            List[MapData]: List of updated MapData objects.
    """
    ...
    if sys_input: 
     ...
     #mark as global const
    actions=[]

    # Create new copy of map data to modify
    _locations = curr_map.get_locations()
    _level_info = curr_map.get_level_info()

    # When commands are empty, mark as invalid input
    if not commands:
        _level_info["invalid_input"] = True
        actions.append(curr_map.update_data(_locations, _level_info))
        return actions
    ...
    for action in commands:
        ...
        if action not in POSSIBLE_INPUTS:
            ...
            _locations = orig_map.get_locations()
            _level_info = orig_map.get_level_info()
        elif action=="P":
            ...
            if 0<=x+i<_level_info["size"][0] and 0<=y+j<_level_info["size"][1] and player_location not in _locations["T"]:
