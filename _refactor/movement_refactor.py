...
def user_input(curr_level_state: LevelState, sys_input: str = "") -> list[LevelState]:
    """ 
    ...
    Parameters
    ----------
    curr_level_state: LevelState 
        Current level information.
    orig_level_state: LevelState
        Original level information, used when "!" is inputted by the user.
    ...
    if not commands:
        curr_level_state.set_invalid_input(True)
        actions.append(curr_level_state.get_state())
        return actions
    ...
    for action in commands:
        ...
        # Revert to original state of the level
            ...
            curr_level_state.reset_state()

        # Try to pick up an item
        elif action=="P" and curr_level_state.is_valid_item_tile():
            curr_level_state.pick_item()
        
        # User input is a movement command
        elif curr_level_state.is_valid_movement(POSSIBLE_INPUTS[action]):
            
            # Retrieve the new possible player location and tile object after moving
            # TODO: add the dot tile after next tile
            new_player_location, next_tile = curr_level_state.next_player_location(POSSIBLE_INPUTS[action])
            
            if next_tile is TILES.TREE_TILE and curr_level_state.get_inventory() is TILES.AXE_ITEM:
                # Remove a tree and clear inventory
                curr_level_state.use_axe(new_player_location)

            elif next_tile is TILES.TREE_TILE and curr_level_state.get_inventory() is TILES.FLAMETHROWER_ITEM:
                # Spread a fire and clear inventory
                curr_level_state.use_fire(new_player_location)

            elif next_tile is TILES.ROCK_TILE:
                # Try to push the rock
                try:
                    curr_level_state.push_rock(new_player_location, POSSIBLE_INPUTS[action])
                except ValueError:
                    continue
                ...
                # End the game -> lose
                curr_level_state.game_lose(new_player_location)
            
            elif next_tile is TILES.MUSHROOM_TILE:
                # Collect the mushroom
                curr_level_state.collect_mushroom(new_player_location)
            
            if curr_level_state.check_win():
                # End the game -> win
                curr_level_state.game_end()
            
            # Update Laro's location
            curr_level_state.set_player_location(new_player_location)
        else:
            continue
