...
def test_validate_locations_valid_cases(valid_locations):
    # === Valid/Normal Cases (Regular input values within acceptable limits) ===
    r = max((i for val in valid_locations.values() for i, j in val))+1
    c = max((j for val in valid_locations.values() for i, j in val))+1
    try:
        validate_locations(r, c, valid_locations)
...
def test_validate_locations_boundary_cases_edge(edge_location, edge_location_expected):
    # === Boundary Cases (Values at the boundaries of the acceptable limits) ===
    ## Other Edge Cases
    if edge_location_expected == "ValueError":
        with pytest.raises(ValueError, match="r and c must be between 1 and 30, inclusive"):
